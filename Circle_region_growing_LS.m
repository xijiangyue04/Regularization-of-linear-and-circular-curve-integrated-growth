% 第一步 实现的直线及圆曲线点云轮廓区域增长算法
%该命令实现的是轮廓点的区域增长算法，在增长的过程中，利用最小二乘进行空间圆曲线拟合，并依据点到圆曲线距离进行增长的判断
       ...
       %current_seed_plane_dis=abs([input_pnts(current_neighbors8_indice(seed_k),1]*pnts_parameter(pose_indice,:)');
         ...
         if size(cluster,1)>3
             ...
             [neighbor_idx1,Pc_dis1]=knnsearch(circle_fit1,tempor_cluster,'k',1); 
              mean_Pcdis=mean(Pc_dis1); %空间圆曲线第一个判断条件
              
             [circle_fit2,circle_radius2,theta2] = space_circle_LS(cluster,resolution);
             [neighbor_idx2,Pc_dis2]=knnsearch(circle_fit2,current_neighbor(:,1:3),'k',1);  %空间圆曲线第二个判断条件
...
%聚类后点云空间直线（圆曲线）拟合误差小于距离分辨率，要聚类的点到前聚类点云直线（圆曲线）距离小于距离分辨率，最邻近点距离小于圆曲线半径
